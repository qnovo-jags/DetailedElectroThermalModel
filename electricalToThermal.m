...
for i = 1:32
    ...
    add_block('algebraicManip_lib/threeMinusOneIntoTwo', blockPath, 'Position', [100 100 250 150]);

    % Connect probe outputs to function inputs (order: ocv, iCell, vCell)
    add_line(modelName, sprintf('%s/1', probeName), sprintf('%s/1', blockName), 'autorouting', 'on'); % ocv → input 3
    add_line(modelName, sprintf('%s/2', probeName), sprintf('%s/2', blockName), 'autorouting', 'on'); % iCell → input 1
    add_line(modelName, sprintf('%s/3', probeName), sprintf('%s/3', blockName), 'autorouting', 'on'); % vCell → input 2
